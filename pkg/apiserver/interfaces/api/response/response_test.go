...
func TestReturnErrorSanitizesGenericError(t *testing.T) {
	...
	require.Equal(t, bcode.ErrServer.BusinessCode, resp.Code)
	require.Equal(t, bcode.ErrServer.Message, resp.Message)
...
func TestReturnErrorSanitizesRecordNotExistError(t *testing.T) {
	...
	resp := decodeBcodeResponse(t, recorder)
	require.Equal(t, bcode.ErrNotFound.BusinessCode, resp.Code)
	require.Equal(t, bcode.ErrNotFound.Message, resp.Message)
...
func TestReturnErrorKeepsBusinessErrorMessage(t *testing.T) {
	...
	ReturnError(c, bcode.ErrApplicationNotExist)

	require.Equal(t, int(bcode.ErrApplicationNotExist.HTTPCode), recorder.Code)
	resp := decodeBcodeResponse(t, recorder)
	require.Equal(t, bcode.ErrApplicationNotExist.BusinessCode, resp.Code)
	require.Equal(t, bcode.ErrApplicationNotExist.Message, resp.Message)
}

func TestReturnErrorUsesExplicitSafeClientMessage(t *testing.T) {
	internalErr := fmt.Errorf("%w: before=secret-old after=secret-new", bcode.ErrApplicationConfig)
	err := bcode.WithSafeClientMessage(internalErr, "component mysql changes StatefulSet immutable field serviceName; explicit migration or recreation is required")
	require.ErrorIs(t, err, bcode.ErrApplicationConfig)

	c, recorder := testGinContext()
	ReturnError(c, err)

	require.Equal(t, int(bcode.ErrApplicationConfig.HTTPCode), recorder.Code)
	resp := decodeBcodeResponse(t, recorder)
	require.Equal(t, bcode.ErrApplicationConfig.BusinessCode, resp.Code)
...
func TestReturnErrorMapsValidatorErrorToApplicationConfig(t *testing.T) {
	...
	resp := decodeBcodeResponse(t, recorder)
	require.Equal(t, bcode.ErrApplicationConfig.BusinessCode, resp.Code)
	require.Equal(t, bcode.ErrApplicationConfig.Message, resp.Message)
}
