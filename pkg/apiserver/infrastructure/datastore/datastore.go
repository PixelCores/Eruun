...
// ComparisonQueryOption defines a scalar comparison filter.
type ComparisonQueryOption struct {
	Key   string
	Value interface{}
}
...
type FilterOptions struct {
	...
	In         []InQueryOption
	NotEqual   []ComparisonQueryOption
	LessThan   []ComparisonQueryOption
...
// ConditionalCompareAndSwap is an optional extension interface for datastores that support
// atomic updates guarded by multiple field conditions.
//
// Implementations should ensure all conditions are applied in a single atomic update statement.
type ConditionalCompareAndSwap interface {
