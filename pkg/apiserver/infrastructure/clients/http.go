...
// ReadURL fetches at most readLimit bytes using the outbound URL policy.
// The caller owns its size limit and decides whether a full limit is acceptable.
func ReadURL(ctx context.Context, rawURL string, policy *spec.URLSecurityPolicySpec, readLimit int64) ([]byte, error) {
	...
	rd := io.LimitReader(resp.Body, readLimit)
