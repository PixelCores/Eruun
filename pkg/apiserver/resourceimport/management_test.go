...
func TestEqualAdoptionReplaySnapshotsTreatsLegacyVersionAsEquivalent(t *testing.T) {
	...
	legacy := importcontract.Snapshot{Version: 1, Namespace: "prod", Resources: []importcontract.ResourceSnapshot{resource}}
	current := legacy
	current.Version = importcontract.SnapshotVersion

	require.True(t, equalAdoptionReplaySnapshots(legacy, current))

	current.Resources = append([]importcontract.ResourceSnapshot(nil), current.Resources...)
	current.Resources[0].PendingRecreation = &importcontract.RecreationClaim{Token: "claim-1"}
...
func TestAssignAdoptedResourceSemantics_SameAppMultiRootDependencyRemainsManaged(t *testing.T) {
	...
	assert.Equal(t, importcontract.OwnershipExclusive, resource.ownership)
	assert.Equal(t, importcontract.DispositionManaged, resource.disposition)
...
func TestAssignAdoptedResourceSemantics_ClusterRBACIsAlwaysExternalPreserved(t *testing.T) {
	for _, kindKey := range []string{importKindClusterRoles, importKindClusterRoleBindings} {
		t.Run(kindKey, func(t *testing.T) {
			...
			assert.Equal(t, importcontract.OwnershipExternal, resource.ownership)
			assert.Equal(t, importcontract.DispositionSharedPreserved, resource.disposition)
		})
	}
}
...
func TestPropagateAdoptedRBACSharing_ClusterBindingPreservesNamespacedServiceAccount(t *testing.T) {
	...
	assert.Equal(t, importcontract.OwnershipShared, serviceAccount.ownership)
	assert.Equal(t, importcontract.DispositionSharedPreserved, serviceAccount.disposition)
	assert.Equal(t, importcontract.OwnershipExternal, clusterRole.ownership)
	assert.Equal(t, importcontract.DispositionSharedPreserved, clusterRole.disposition)
	assert.Equal(t, importcontract.OwnershipExternal, clusterBinding.ownership)
	assert.Equal(t, importcontract.DispositionSharedPreserved, clusterBinding.disposition)
}
...
func TestPropagateAdoptedSharedDependencies_ClusterBindingPreservesServiceAccountSecrets(t *testing.T) {
	...
	assert.Equal(t, importcontract.DispositionSharedPreserved, serviceAccount.disposition)
	assert.Equal(t, importcontract.DispositionSharedPreserved, tokenSecret.disposition)
	assert.Equal(t, importcontract.DispositionSharedPreserved, registrySecret.disposition)
}
...
func TestPropagateAdoptedRBACSharing_ClusterBindingClassificationIsDeterministic(t *testing.T) {
	...
	for range 64 {
		...
		assert.Equal(t, importcontract.OwnershipShared, serviceAccount.ownership)
		assert.Equal(t, importcontract.DispositionSharedPreserved, serviceAccount.disposition)
		assert.Equal(t, importcontract.OwnershipShared, roleBinding.ownership)
		assert.Equal(t, importcontract.DispositionSharedPreserved, roleBinding.disposition)
		assert.Equal(t, importcontract.OwnershipShared, role.ownership)
		assert.Equal(t, importcontract.DispositionSharedPreserved, role.disposition)
	}
}
...
func TestImportNamespaceResources_AdoptedFiveExactRootsDependencyClosureAndSafeApply(t *testing.T) {
	...
	assert.Equal(t, importcontract.OwnershipExclusive, mysqlResult.Ownership)
	assert.Equal(t, importcontract.DispositionManaged, mysqlResult.Disposition)
	...
	assert.Equal(t, importcontract.OwnershipDataProtected, pvcResult.Ownership)
	assert.Equal(t, importcontract.DispositionDataProtected, pvcResult.Disposition)
	...
	assert.Equal(t, importcontract.OwnershipShared, ingressResult.Ownership)
	assert.Equal(t, importcontract.DispositionSharedPreserved, ingressResult.Disposition)

	serviceResult := requireImportResourceResult(t, dryRun, "Service", backendService.Name)
	assert.Equal(t, importcontract.OwnershipShared, serviceResult.Ownership)
	assert.Equal(t, importcontract.DispositionSharedPreserved, serviceResult.Disposition)
	secretResult := requireImportResourceResult(t, dryRun, "Secret", mysqlSecret.Name)
	assert.Equal(t, importcontract.OwnershipShared, secretResult.Ownership)
	assert.Equal(t, importcontract.DispositionSharedPreserved, secretResult.Disposition)
	tlsSecretResult := requireImportResourceResult(t, dryRun, "Secret", tlsSecret.Name)
	assert.Equal(t, importcontract.OwnershipShared, tlsSecretResult.Ownership)
	assert.Equal(t, importcontract.DispositionSharedPreserved, tlsSecretResult.Disposition)
	registrySecretResult := requireImportResourceResult(t, dryRun, "Secret", registrySecret.Name)
	assert.Equal(t, importcontract.OwnershipExclusive, registrySecretResult.Ownership)
	assert.Equal(t, importcontract.DispositionManaged, registrySecretResult.Disposition)
	...
	for _, resource := range []struct {
		...
		assert.Equal(t, importcontract.OwnershipExclusive, result.Ownership)
		assert.Equal(t, importcontract.DispositionManaged, result.Disposition)
	}

	pvResult := requireImportResourceResult(t, dryRun, "PersistentVolume", persistentVolume.Name)
	assert.Equal(t, importcontract.OwnershipExternal, pvResult.Ownership)
	assert.Equal(t, importcontract.DispositionExcluded, pvResult.Disposition)

	proxyResult := requireImportResourceResult(t, dryRun, "Deployment", proxy.Name)
	assert.Equal(t, importcontract.OwnershipExternal, proxyResult.Ownership)
	assert.Equal(t, importcontract.DispositionExcluded, proxyResult.Disposition)
...
func TestImportNamespaceResources_AdoptedPlanDriftAndHPATargetWriteNothing(t *testing.T) {
	...
	t.Run("HPA target blocks the entire adopted apply", func(t *testing.T) {
		...
		assert.Equal(t, importcontract.DispositionBlocked, result.Disposition)
...
func TestImportNamespaceResources_LegacyObserveNeverLabelsSkippedVCTStatefulSetOrPVC(t *testing.T) {
	...
	assert.Equal(t, importcontract.DispositionBlocked, stsResult.Disposition)
