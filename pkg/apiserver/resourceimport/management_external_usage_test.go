...
func TestImportNamespaceResources_AdoptedTargetDeploymentOwnerChainIsNotExternal(t *testing.T) {
	...
	for _, resourceIdentity := range []struct {
		...
		assert.Equal(t, importcontract.OwnershipExclusive, result.Ownership, resourceIdentity)
		assert.Equal(t, importcontract.DispositionManaged, result.Disposition, resourceIdentity)
	}
	pvc := requireImportResourceResult(t, response, "PersistentVolumeClaim", "api-data")
	assert.Equal(t, importcontract.OwnershipDataProtected, pvc.Ownership)
	assert.Equal(t, importcontract.DispositionDataProtected, pvc.Disposition)
...
func TestImportNamespaceResources_AdoptedPreservesSourceReplicasIncludingZero(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.Equal(t, test.want, appService.createReqs[0].Components[0].Replicas)
...
func assertAdoptedExternalUsageShared(
	...
	for _, resourceIdentity := range []struct {
		...
		result := requireImportResourceResult(t, response, resourceIdentity.kind, resourceIdentity.name)
		assert.Equal(t, importcontract.OwnershipShared, result.Ownership, resourceIdentity)
		assert.Equal(t, importcontract.DispositionSharedPreserved, result.Disposition, resourceIdentity)
	}
}
