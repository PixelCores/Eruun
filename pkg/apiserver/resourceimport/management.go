...
type adoptedCanonicalPlanApp struct {
	...
	Snapshot    importcontract.Snapshot     `json:"snapshot"`
}
...
type adoptedCanonicalComponent struct {
	Name     string                          `json:"name"`
	Workload importcontract.ResourceIdentity `json:"workload"`
}

func (s *serviceImpl) tryAdoptedApplyReplay(
	ctx context.Context,
	namespace string,
...
func decodeAdoptionSnapshot(value *model.JSONStruct) (importcontract.Snapshot, error) {
	if value == nil {
		return importcontract.Snapshot{}, fmt.Errorf("snapshot is missing")
	}
	payload, err := json.Marshal(value)
	if err != nil {
		return importcontract.Snapshot{}, err
	}
...
func equalAdoptionReplaySnapshots(left, right importcontract.Snapshot) bool {
	normalize := func(snapshot importcontract.Snapshot) ([]byte, error) {
		...
		snapshot.Resources = append([]importcontract.ResourceSnapshot(nil), snapshot.Resources...)
...
func (s *serviceImpl) loadAdoptedImportKeyring() (*importsecret.Keyring, error) {
	if s.Cfg == nil {
		return nil, nil
	}
...
func (s *serviceImpl) prepareAdoptedPlansForExecution(
	ctx context.Context,
	namespace string,
	plans []importAppPlan,
) {
...
func (s *serviceImpl) buildAdoptedImportPlanning(
	ctx context.Context,
	namespace string,
	resources []*importResource,
	...
	for _, mapping := range mappings {
		...
		if reasons := uniqueSortedStrings(blockedPlanReasons[planKey]); len(reasons) > 0 {
			for _, resource := range plan.resources {
				resource.disposition = importcontract.DispositionBlocked
...
func (s *serviceImpl) collectAdoptedExternalPodAndReplicaSetUsage(
	ctx context.Context,
	namespace string,
	roots []*adoptedRoot,
	membership map[string]*adoptedMembership,
...
func assignAdoptedResourceSemantics(
	...
	for _, member := range membership {
		...
		if resource.kindKey == importKindClusterRoles ||
			...
			resource.ownership = importcontract.OwnershipExternal
			resource.disposition = importcontract.DispositionSharedPreserved
			continue
		}
		...
		if len(member.externalWorkloads) > 0 {
			resource.ownership = importcontract.OwnershipShared
			resource.disposition = importcontract.DispositionSharedPreserved
			continue
		}
		if resource.kindKey == importKindPersistentVolumeClaims {
			resource.ownership = importcontract.OwnershipDataProtected
			resource.disposition = importcontract.DispositionDataProtected
			continue
		}
...
func markExcludedAdoptedResources(resources []*importResource, membership map[string]*adoptedMembership) []*importResource {
	...
	for _, resource := range resources {
		...
		resource.dependencyRole = adoptedDependencyRoleForResource(resource)
		resource.ownership = importcontract.OwnershipExternal
		resource.disposition = importcontract.DispositionExcluded
...
func (s *serviceImpl) buildAdoptedCanonicalPlan(
	ctx context.Context,
	namespace string,
	plans []importAppPlan,
) ([]byte, error) {
...
func (s *serviceImpl) adoptedCanonicalTargetState(
	ctx context.Context,
	targetAppID string,
...
func (s *serviceImpl) scanAdoptedHPATargets(ctx context.Context, namespace string) (map[string][]string, error) {
...
func initialImportResourceStatus(resource *importResource) string {
	...
	case importcontract.DispositionBlocked, importcontract.DispositionExcluded:
		return importResourceStatusSkipped
	default:
...
func (s *serviceImpl) mutateAdoptedApplicationCreate(
	ctx context.Context,
