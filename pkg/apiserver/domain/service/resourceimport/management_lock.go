...
func (s *serviceImpl) withAdoptedNamespaceApplyLock(
	...
	if mutex == nil {
		return nil, fmt.Errorf("%w: resource import management locker returned no mutex", bcode.ErrDistributedLockUnavailable)
	}
	if err := mutex.Lock(ctx); err != nil {
		if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
			return nil, err
		}
		return nil, fmt.Errorf("%w: acquire resource import management lock for namespace %s: %v", bcode.ErrDistributedLockUnavailable, namespace, err)
	}
	...
	cleanup()
	if renewErr != nil {
		return nil, fmt.Errorf("%w: renew resource import management lock for namespace %s: %v", bcode.ErrDistributedLockUnavailable, namespace, renewErr)
	}
	if runErr != nil {
		return nil, runErr
	}
	if unlockErr != nil {
		return nil, fmt.Errorf("%w: release resource import management lock for namespace %s: %v", bcode.ErrDistributedLockUnavailable, namespace, unlockErr)
	}
	return response, nil
}
