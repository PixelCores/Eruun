...
func TestUpdateVersionNoopAutoExecRollsBackWhenCallbackRecordsFail(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Error(t, err)
			require.Contains(t, err.Error(), "record update-version task")
...
func TestUpdateVersionAutoExecFalseIgnoresInvalidTaskCallback(t *testing.T) {
	...
	require.Empty(t, store.tasks[resp.TaskID].WorkflowID)
	require.Nil(t, store.tasks[resp.TaskID].Callback)
}
