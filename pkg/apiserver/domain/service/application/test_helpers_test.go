...
func (m *mockAppRepo) FindByName(ctx context.Context, name string) (*model.Applications, error) {
	...
	apps, err := m.ListByQuery(ctx, &model.Applications{Name: name}, datastore.ListOptions{Page: 1, PageSize: 1})
	if err != nil {
		return nil, err
	}
	if len(apps) == 0 {
...
func (m *mockCleanupAppRepo) FindByIDs(_ context.Context, ids []string) ([]*model.Applications, error) {
	applications := make([]*model.Applications, 0, len(ids))
	for _, id := range ids {
		if app, ok := m.store.applications[id]; ok {
			applications = append(applications, app)
...
func (m *mockCleanupAppRepo) ListByQuery(ctx context.Context, query *model.Applications, options datastore.ListOptions) ([]*model.Applications, error) {
	return nil, nil
}
...
func (m *mockCleanupComponentRepo) Update(ctx context.Context, component *model.ApplicationComponent) error {
	for i, c := range m.store.components {
		if c.Name == component.Name && c.AppID == component.AppID {
			m.store.components[i] = component
			return nil
		}
	}
	return nil
...
func holdApplicationTestAppScheduleLock(t *testing.T, lockProvider locker.Locker, appID string) func() {
	t.Helper()
...
func setTestURLSecurityPolicy(t testing.TB, store *inMemoryAppStore, policy spec.URLSecurityPolicySpec) {
	t.Helper()
	if store.settings == nil {
		store.settings = make(map[string]*model.SystemSetting)
	}
