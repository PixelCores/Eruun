...
func TestAdoptedCleanupPlanApplyDeletesExclusiveAndRetainsPVC(t *testing.T) {
	...
	store.apps["app-1"] = adoptedCleanupApplication(t, []adoption.ResourceSnapshot{
		adoptedCleanupSnapshotResource("apps/v1", "Deployment", deployment.Name, string(deployment.UID), "workload", adoption.OwnershipExclusive, adoption.DispositionManaged),
		adoptedCleanupSnapshotResource("v1", "PersistentVolumeClaim", pvc.Name, string(pvc.UID), "pvc", adoption.OwnershipDataProtected, adoption.DispositionDataProtected),
	})
...
func TestAdoptedCleanupDeletesSignedRuntimeChildren(t *testing.T) {
	...
	testCases := []struct {
		name                      string
		root                      runtime.Object
		rootSnapshot              adoption.ResourceSnapshot
		rootResource              string
...
func TestAdoptedCleanupRescansSharingAfterRootDeletion(t *testing.T) {
	...
	store.apps["app-1"] = adoptedCleanupApplication(t, []adoption.ResourceSnapshot{
		adoptedCleanupSnapshotResource("apps/v1", "Deployment", deployment.Name, string(deployment.UID), "workload", adoption.OwnershipExclusive, adoption.DispositionManaged),
		adoptedCleanupSnapshotResource("v1", "Secret", secret.Name, string(secret.UID), "secret", adoption.OwnershipExclusive, adoption.DispositionManaged),
	})
...
func TestAdoptedCleanupBlocksStatefulSetWhenDeletedPolicy(t *testing.T) {
	...
	store.apps["app-1"] = adoptedCleanupApplication(t, []adoption.ResourceSnapshot{
		adoptedCleanupSnapshotResource("apps/v1", "StatefulSet", statefulSet.Name, string(statefulSet.UID), "workload", adoption.OwnershipExclusive, adoption.DispositionManaged),
	})

	plan, err := service.PlanApplicationResourceCleanup(context.Background(), "app-1")
	require.NoError(t, err)
	require.Equal(t, adoption.DispositionBlocked, plan.ResourceResults[0].Disposition)
...
func TestAdoptedCleanupRetainsSecretThatBecameSharedAfterImport(t *testing.T) {
	...
	require.Equal(t, adoption.OwnershipShared, plan.ResourceResults[0].Ownership)
	require.Equal(t, adoption.DispositionSharedPreserved, plan.ResourceResults[0].Disposition)
...
func TestAdoptedCleanupRetainsSecretReferencedByExternalServiceAccount(t *testing.T) {
	...
	store.apps["app-1"] = adoptedCleanupApplication(t, []adoption.ResourceSnapshot{
		adoptedCleanupSnapshotResource("v1", "Secret", secret.Name, string(secret.UID), "secret", adoption.OwnershipExclusive, adoption.DispositionManaged),
		adoptedCleanupSnapshotResource("v1", "ServiceAccount", serviceAccount.Name, string(serviceAccount.UID), "service-account", adoption.OwnershipExclusive, adoption.DispositionManaged),
	})
	...
	for _, result := range plan.ResourceResults {
		require.Equal(t, adoption.OwnershipShared, result.Ownership)
		require.Equal(t, adoption.DispositionSharedPreserved, result.Disposition)
	}
...
func TestAdoptedCleanupRetainsRoleReferencedThroughNewClusterBinding(t *testing.T) {
	...
	store.apps["app-1"] = adoptedCleanupApplication(t, []adoption.ResourceSnapshot{
		adoptedCleanupSnapshotResource("v1", "ServiceAccount", serviceAccount.Name, string(serviceAccount.UID), "service-account", adoption.OwnershipExclusive, adoption.DispositionManaged),
		adoptedCleanupSnapshotResource("rbac.authorization.k8s.io/v1", "Role", role.Name, string(role.UID), "rbac", adoption.OwnershipExclusive, adoption.DispositionManaged),
		adoptedCleanupSnapshotResource("rbac.authorization.k8s.io/v1", "RoleBinding", roleBinding.Name, string(roleBinding.UID), "rbac", adoption.OwnershipExclusive, adoption.DispositionManaged),
	})
...
func TestAdoptedCleanupRetainsTLSSecretWhenAdoptedIngressBecomesShared(t *testing.T) {
	...
	store.apps["app-1"] = adoptedCleanupApplication(t, []adoption.ResourceSnapshot{
		adoptedCleanupSnapshotResource("v1", "Secret", secret.Name, string(secret.UID), "secret", adoption.OwnershipExclusive, adoption.DispositionManaged),
		adoptedCleanupSnapshotResource("v1", "Service", serviceResource.Name, string(serviceResource.UID), "service", adoption.OwnershipExclusive, adoption.DispositionManaged),
		adoptedCleanupSnapshotResource("networking.k8s.io/v1", "Ingress", ingress.Name, string(ingress.UID), "ingress", adoption.OwnershipExclusive, adoption.DispositionManaged),
	})
...
func adoptedCleanupApplication(t *testing.T, resources []adoption.ResourceSnapshot) *model.Applications {
	t.Helper()
	snapshot := adoption.NewSnapshot("prod", resources)
...
func adoptedCleanupSnapshotResource(
	...
	return adoption.ResourceSnapshot{
