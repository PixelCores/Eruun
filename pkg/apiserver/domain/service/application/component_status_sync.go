...
func findComponentForStatusSync(ctx context.Context, store datastore.DataStore, appID string, componentID int) (*model.ApplicationComponent, error) {
	...
	entities, err := store.List(ctx, query, &datastore.ListOptions{})
	if err != nil {
		return nil, err
	}
...
func SyncComponentStatus(ctx context.Context, store datastore.DataStore, componentCache cache.ICache, update *informer.ComponentStatusUpdate) {
	...
	defer func() {
		if componentCache == nil || componentCache.IsCacheDisabled() {
			return
		}
	...
	component, err := findComponentForStatusSync(ctx, store, appID, update.ComponentID)
	if err != nil {
	...
	if component.Status == string(config.ComponentStatusCleaning) {
		if update.Replicas != nil && *update.Replicas == 0 {
			...
			lastAbnormal := ""
			updated, err := repository.UpdateComponentRuntimeFieldsIfUnchanged(ctx, store, &expected, map[string]interface{}{
	...
	updated, err := repository.UpdateComponentRuntimeFieldsIfUnchanged(ctx, store, &expected, runtimeUpdates)
	if err != nil {
