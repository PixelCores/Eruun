	...
	for idx, policy := range specs {
		...
		sa := &corev1.ServiceAccount{
			ObjectMeta: metav1.ObjectMeta{
				Name:        saName,
				Namespace:   namespace,
				Labels:      naming.NormalizeLabelValues(policy.ServiceAccountLabels),
		...
		if policy.ClusterScope {
			role := &rbacv1.ClusterRole{
				ObjectMeta: metav1.ObjectMeta{
					Name:   roleName,
					Labels: naming.NormalizeLabelValues(policy.RoleLabels),
				},
				Rules: rules,
			}
			binding := &rbacv1.ClusterRoleBinding{
				ObjectMeta: metav1.ObjectMeta{
					Name:   bindingName,
					Labels: naming.NormalizeLabelValues(policy.BindingLabels),
				},
