...
func TestResourceImportGenerationFailurePersistsSafeClientReason(t *testing.T) {
	task := &model.WorkflowQueue{
		TaskID:             "resource-import-generation-failure",
		WorkspaceID:        "workspace",
		Type:               config.WorkflowTaskTypeResourceImportScan,
		Status:             config.StatusQueued,
		ResourceActionInfo: `{"version":2,"namespace":"team-production","request":{}}`,
	}
	store := &controllerTestStore{}
	ctl := newTestWorkflowController(t, task, nil, store)
	ctl.accountConfig = &spec.AccountConfig{}
	ctl.workspaceManager = &workspace.Manager{}
	ctl.workspace = &model.Workspace{ID: "workspace", Namespace: "team-production"}

	require.Error(t, ctl.run(context.Background(), 1))
	require.Equal(t, config.StatusFailed, ctl.snapshotTask().Status)
	require.Equal(t, importcontract.PreExecutionFailureReason, ctl.snapshotTask().SchedulingReason)
	store.mu.Lock()
	defer store.mu.Unlock()
