...
func TestDeployNamespacedRBACJobCtlRunAdoptedPreservedDispositionNeverTouchesKubernetes(t *testing.T) {
	testCases := []struct {
		...
		{
			name:        "data protected",
			ownership:   adoption.OwnershipDataProtected,
			disposition: adoption.DispositionDataProtected,
		},
		{
			name:        "blocked",
...
func TestAdoptedNamespacedRBACRecreateFromSnapshotAndRotateUID(t *testing.T) {
	t.Run("service account", func(t *testing.T) {
		...
		saved := adoptedSnapshotResource(t, source, "backend", "service-account", adoption.OwnershipExclusive, adoption.DispositionManaged)
		store := &adoptedSourceStore{app: adoptedApplication(t, "app-1", "ops", saved)}
		client := fake.NewSimpleClientset()
	...
	t.Run("role binding", func(t *testing.T) {
		...
		saved := adoptedSnapshotResource(t, source, "backend", "role-binding", adoption.OwnershipExclusive, adoption.DispositionManaged)
		store := &adoptedSourceStore{app: adoptedApplication(t, "app-1", "ops", saved)}
		client := fake.NewSimpleClientset()
...
func TestAdoptedRoleRecreationPersistenceFailureRetainsLiveObjectAndPendingClaim(t *testing.T) {
	...
	saved := adoptedSnapshotResource(t, source, "backend", "role", adoption.OwnershipExclusive, adoption.DispositionManaged)
	store := &adoptedSourceStore{
		app:                        adoptedApplication(t, "app-1", "ops", saved),
