...
| 独立 Job `type/spec/traits/resultPolicy` | `POST /jobs` 请求；可省略 workspaceId，显式填写时必须匹配认证空间 | `WorkflowQueue.Type=job`，`JobSpec` 保存已校验规格；`JobInfo.Type` 为 command/eval | 无 | 固定空间 namespace，TaskID/执行代映射 Job 与 Pod 注解 | `JobToken` 与执行 checkpoint 不对用户序列化 | 复用队列与一次性 Job；不创建占位应用或平行调度器 |
