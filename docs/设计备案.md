...
> 状态：Historical / Audit。本文保留早期问题和设计思路，不代表当前实现或未来 AI Runtime 的已决策契约。
...
> Eruun 会借鉴OAM模型中的"组件"思想，通过组件化的拆分，将应用由一个或者多个组件构成。这里的每个组件在部署/更新时都会转换为不同类型的Job，通过模型的策略来决定系统的工作流引擎执行时是串行还是并行。
...
8.Kubernetes的核心思想是声明式的，用户应该关心“我需要什么”，而不是应该运行一个Job来描述我想要的东西是什么，在Eruun中应该做的更加友好一点，可以使用一种混合模式来实现：
...
Storage支持多种存储方式，比如`persistent`稳定存储，它对应的是Kubernetes中的 PersistentVolumeClaim 和 VolumeMount 属性。在Eruun中为了简化表达使用: `{"type":"persistent","mountPath":"/data","size": "20"}`Json格式对组件提出存储方面的附加追述。
...
| config       | ConfigMap + VolumeMount 或 EnvFrom  | 如果在Storage特征中定义config那么Eruun会将其视为:在Pod中将ConfigMap当作文件使用.<br />注意，必须先创建ConfigMap，才能使用它，ConfigMap总 |
| secret       | Secret + VolumeMount 或 EnvFrom     | 如果在Storage特征中定义secret那么Eruun会将其视为:在Pod中将secret当作文件使用 |
...
在Eruun是如何实现上述场景的？使用特征中的Envs结构来描述：
...
Eruun 中将这种配置项当作文件使用的方式，使用特征中的**Storage**来描述它：
...
在Eruun中使用**EnvFrom**来描述这种将ConfigMap当作变量注入
...
本项目通过 K8s Lease(调度单例) + Redis Streams 消费组(消费互斥)来实现无须显式分布式锁的一致性方案。该方案关键是应用层幂等和错误处理策略。
