...
> 状态：Implemented Reference。本文展示当前架构关系与流程图，具体运行参数以代码和 `docs/README.md` 为准。
...
flowchart TB
    ...
    class KubeClient,RedisClient,KafkaClient,DatastoreIF,MySQLImpl,QueueIF,RedisStreams,KafkaQueue infra
```
...
### 4.2 Redis Streams 消息后端架构图

```mermaid
flowchart TB
    subgraph Scheduler["Scheduler 角色"]
        Dispatcher[Waiting Task Dispatcher]
        LeaderElection[Scheduler Leader Election<br/>Kubernetes Lease]
    end
    ...
    subgraph Redis["Redis"]
        Stream[("Redis Stream<br/>eruun.workflow.dispatch")]
...
flowchart TB
    ...
    subgraph Kafka["Kafka Cluster"]
        Topic[("Topic: eruun.workflow.dispatch")]
        P0[Partition 0]
        P1[Partition 1]
        P2[Partition 2]
        CG[Consumer Group<br/>eruun-workflow-workers]
    end
...
### 4.4 Redis Streams 与 Kafka 后端对比

```mermaid
flowchart LR
    subgraph Redis["Redis Streams 后端"]
        direction TB
        R_API[API] --> R_DB[(MySQL)]
        R_DB --> R_Dispatcher[Scheduler<br/>Leader]
        R_Dispatcher --> R_Queue[(Redis Streams)]
        R_Queue --> R_Workers[Workers<br/>多实例]
        R_Workers --> R_K8s[Kubernetes]
    end
    
    subgraph Kafka["Kafka 后端"]
        direction TB
        K_API[API] --> K_DB[(MySQL)]
        K_DB --> K_Dispatcher[Scheduler<br/>Leader]
        K_Dispatcher --> K_Queue[(Kafka)]
        K_Queue --> K_Workers[Workers<br/>多实例]
        K_Workers --> K_K8s[Kubernetes]
    end

    classDef backend fill:#e1f5fe,stroke:#01579b

    class R_API,R_DB,R_Dispatcher,R_Queue,R_Workers,R_K8s,K_API,K_DB,K_Dispatcher,K_Queue,K_Workers,K_K8s backend
```
...
flowchart TB
    ...
    subgraph StatusSync["状态同步"]
        ...
        DB[(MySQL<br/>eruun_app_components)]
    end
...
```sql
ALTER TABLE eruun_app_components 
...
为减少内存消耗，Informer 仅监听带有 `eruun.io/app-id` 标签的 Pod：
...
s.InformerManager = informer.NewManager(
    ...
    informer.WithLabelSelector(config.LabelAppID),  // 只监听 Eruun 管理的资源
)
```

这样 Informer 不会缓存集群中其他应用的 Pod，显著降低内存占用。
...
- Deployment / StatefulSet 的工作负载就绪等待依赖 `ResourceReadyWaiter` 的 Informer 事件；缺少 waiter 会直接失败，不自动降级为轮询。
- PVC / Service 等资源仍使用轮询式等待路径。
- cleanup、database reset 等删除或重建流程也会按具体资源语义使用轮询等待。

因此本文中的事件驱动与轮询对比是架构参考，不表示所有 `wait()` 都有统一的 Informer -> polling fallback。
...
stateDiagram-v2
    ...
    queued --> waiting: 分发租约过期 (Scheduler reaper)
    
    running --> completed: 执行成功
    running --> failed: 执行失败
    running --> timeout: 执行超时
    running --> cancelled: 用户取消
    running --> waiting: 执行租约过期 (Scheduler reaper)
