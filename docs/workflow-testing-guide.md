...
> 状态：Historical / Audit。本文是工作流手工测试场景集合，最新接口入口以 `docs/README.md` 和 API 文档为准。


## 概述

本文档详细描述了Eruun工作流系统的完整测试方案，旨在验证工作流的稳定性和可靠性。测试覆盖从基础组件创建到复杂多组件部署的全生命周期管理。
...
```shell
curl http://{{127.0.0.1:8000}}/api/v1/applications/{{:APP_ID}}/workflow/exec

curl -X POST \
  http://127.0.0.1:8000/api/v1/applications/7mcor3r4su789r99jhpxyzat/workflow/exec \
...
- [x] Service正确配置
  ...
  metadata:
    ...
    labels:
      ...
      eruun.io/app-id: 7mcor3r4su789r99jhpxyzat
      eruun.io/component-id: "5"
  ...
  "component": [
    {
      ...
      "properties": {
        ...
        "env": {
          ...
          "MYSQL_DATABASE": "demo",
          "MYSQL_USER": "demo",
          "MYSQL_PASSWORD": "__REPLACE_WITH_SECRET__"
        }
      },
      "traits": {
        "storage": [
...
2. 触发 store 工作流：
   ```shell
   curl -X POST \
     http://127.0.0.1:8000/api/v1/applications/${APP_ID}/workflow/exec \
     -H 'Content-Type: application/json' \
...
3. 轮询任务状态直到结束：
   ```shell
   curl http://127.0.0.1:8000/api/v1/workflow/tasks/${TASK_ID}/status
   ```
...
- [x] MySQL读写校验
  ...
  kubectl run mysql-client --rm -it \
      ...
      mysql -h svc-mysql-primary-4tbupjg43ln3yj249l0v0fv8 -uroot -p__REPLACE_WITH_SECRET__ \
...
## StatefulSet PVC 回收策略（默认行为）

- 对于 `store` 组件通过 `volumeClaimTemplates` 动态创建的 PVC，默认使用：
  - `whenDeleted: Delete`
  - `whenScaled: Retain`
- 因此删除 StatefulSet 时，这类模板 PVC 会被一并清理。
- 非 `volumeClaimTemplates` 的独立 PVC 不受该策略影响，仍按原有清理逻辑处理。
...
- [x] 数据内容正确
  ...
  Name:         app-config
  Namespace:    default
  Labels:       app.kubernetes.io/managed-by=eruun
                eruun.io/app-id=4kwaenmqb055rt6pyyp8ouh7
                eruun.io/component-id=8
                eruun.io/test-case=TC003
  Annotations:  <none>
...
- [x] 数据正确编码
  ...
  {"password":"__REPLACE_WITH_SECRET__","username":"YWRtaW4="}%
  ```
- [x] 可以被其他组件引用
- [x] 工作流状态为completed
...
- [x] 新版本Pod正常启动
  ```shell
  kubectl get pods -l eruun.io/app-id=${APP_ID} -o wide
  ```
- [x] 旧版本Pod平滑下线
- [x] 服务不中断
- [x] 更新状态为completed
...
```json
curl -X POST "http://localhost:8000/api/v1/applications/kyysru4h07l1w7ghqd6mkhcx/version" \
  -H "Content-Type: application/json" \
  -d '{
    "version": "1.2.0",
    "strategy": "rolling",
...
- [x] PVC自动创建
  ```shell
  kubectl get pvc -l eruun.io/app-id=${APP_ID}
...
- [x] Volume正确挂载
  ```shell
  kubectl describe pod -l eruun.io/app-id=${APP_ID} | grep -A5 'Volumes'
...
- [x] ConfigMap正确挂载
  ```shell
  kubectl describe pod -l eruun.io/app-id=${APP_ID} | grep -A5 'Mounts'
  ...
  "component": [
    {
      ...
      "properties": {
        "secret": {
          "db-password": "__REPLACE_WITH_SECRET__",
          "api-key": "__REPLACE_WITH_SECRET__"
        }
      }
    },
    {
...
- [x] Ingress成功创建
  ```shell
  kubectl get ingress -l eruun.io/app-id=${APP_ID}
...
- [x] Pod使用正确的SA
  ```shell
  kubectl get pod -l eruun.io/app-id=${APP_ID} -o jsonpath='{.items[0].spec.serviceAccountName}'
...
- [ ] 并行执行正确
  ```shell
  kubectl get pods -l eruun.io/app-id=${APP_ID} --watch
  ```
- [ ] 资源竞争处理
- [ ] 状态聚合正确
- [ ] 错误隔离有效
...
- [ ] 所有组件创建成功
  ```shell
  kubectl get all -l eruun.io/app-id=${APP_ID}
  ```
- [ ] 网络配置正确
  ```shell
  kubectl get svc -l eruun.io/app-id=${APP_ID}
  ```
- [ ] 服务发现正常
- [ ] 数据存储正确
  ```shell
...
- [ ] 状态更新为failed
  ```shell
  kubectl get pods -l eruun.io/app-id=${APP_ID}
  # Pod 应处于 ImagePullBackOff 状态
  ```
- [ ] 资源清理执行
- [ ] 错误信息详细
  ```shell
  kubectl describe pod -l eruun.io/app-id=${APP_ID} | grep -A5 'Events'
  ```
- [ ] 重试机制有效
...
- [ ] 错误信息准确
  ```shell
  kubectl describe pod -l eruun.io/app-id=${APP_ID} | grep -A10 'Events'
...
```shell
# 并发发送两个更新请求
curl -X POST "http://127.0.0.1:8000/api/v1/applications/${APP_ID}/version" \
  -H 'Content-Type: application/json' \
...
- [ ] 性能不降级
  ```shell
  time curl -X POST http://127.0.0.1:8000/api/v1/applications \
    -H 'Content-Type: application/json' \
    -d @payloads/large-scale.json
  ```
- [ ] 内存使用合理
  ```shell
  kubectl top pods -l eruun.io/app-id=${APP_ID}
  ```
- [ ] 并发控制有效
- [ ] 状态管理正确
  ```shell
  kubectl get all -l eruun.io/app-id=${APP_ID} --show-labels
  ```
- [ ] 资源清理完整
...
- [ ] 引用关系正确
  ```shell
  kubectl describe pod -l eruun.io/app-id=${APP_ID} | grep -A10 'Environment Variables from'
  ```
- [ ] 错误处理适当
  ```shell
  # 使用无效命名的请求应返回 400 错误
...
#### 测试项 TC023：单次克隆模板（templateEnabled=true）
- 前置：模板应用已存在且 `templateEnabled=true`，包含 store + secret 组件。
- 请求示例（覆盖 env/secret）：
```json
{
...
- 验证：
  ...
  - 模板 `templateEnabled=true` 方可引用，`templateEnabled=false` 返回 400。

#### 测试项 TC024：模板禁用/ID 缺失错误
- 模板 `templateEnabled=false` 或 `tmp.id` 为空/不存在，返回 400/404，错误信息分别为 `template application is not enabled` / `template id is required` / `application not found`。

#### 测试项 TC025：同模板多条覆盖匹配
- 同一 `tmp.id` 多条目仅用于覆盖（类型优先匹配），未匹配的模板组件按 baseName 或模板名生成新名称。
