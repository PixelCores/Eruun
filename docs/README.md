...
- 独立 `command` Job 与 `type: job` + `traits.eval` 评测使用空间授权；评测也支持 Application/Workflow 组件，Runner 使用 Harbor 0.22.0；见 `workspace-jobs-api.md`。当前没有通用 Agent 注册、MCP、向量化、vLLM/HAMi 或托管 AI Provider 公共 API。
...
| `pkg/apiserver/jobs` | 空间独立 Job、评测任务准备与评测数据 | command、Harbor 提交、原生任务包、完整结果、独立保存及保留策略 | 复用现有 WorkflowQueue、JobInfo 和执行租约；公共规格在 `domain/spec`，command Job 构建在本包 `command_builder.go`，框架 Runner 在 `pkg/apiserver/jobs/runners/harbor` |
...
| `pkg/apiserver/domain/spec` | 共享规格、资源契约、策略和校验（含 Aliyun 设置脱敏） | Auth、OAuth、URL 安全、云资源配置、资源类型、Service 暴露类型与共享策略 | 业务取值及归一化与对应规格集中定义；Storage/Volume 类型与映射集中在 `storage.go`；应用管理模式、更新策略、执行范围与组件动作集中在 `application_policy.go` |
...
| `pkg/apiserver/event/workflow/cloudjob` | 云资源 Provider 合约与实现 | 云资源步骤、Provider 注册、外部云动作 | 合约字符串集中为常量；Provider 可选设置支持由 `domain/service/systemsetting` 管理 |
...
| 存量 Kubernetes 资源导入 | `pkg/apiserver/domain/service/resourceimport` | 一次性规则扫描、候选选择、异步纳管 Job、导入运行期协调 | `import-existing-namespace-api.md` |
