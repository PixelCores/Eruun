...
- 设置页 v1 只管理 `eruun_system_setting` 当前支持的七种 `type`：`nodeSelector`、`rbacPolicies`、`apiAuth`、`oauthAuth`、`aliyunCloud`、`urlSecurityPolicy`、`podRestartMonitor`。
...
- `projectName`、`defaultNamespace`、默认资源限制不是当前 system setting 类型；如果前端需要这些值，只能作为前端本地偏好或创建应用表单默认值，不能提交到 `/api/v1/settings`。
...
| Access Control | `apiAuth` | `enabled`、JWT 配置、`authorization.defaultEffect`、`authorization.routes` | 可编辑；密钥字段保存时必须重新输入真实值 |
...
| OAuth | `oauthAuth` | Google OAuth、JWT 签发、角色映射 | 建议只展示配置状态或禁用启用入口；当前 OAuth 路由未注册 |
...
- `apiAuth`、`urlSecurityPolicy`、`podRestartMonitor` 启动时会自动补齐默认记录，通常直接 `PUT`。
- `nodeSelector`、`rbacPolicies`、`oauthAuth` 可由初始化 SQL 补齐；前端遇到 404 时再 `POST` 创建。
...
- `jwt.algorithms`
- `jwt.hs256.secret`
- `jwt.rs256.publicKeys`
- `authorization.defaultEffect`
- `authorization.routes`

读取响应会对 `jwt.hs256.secret` 脱敏为 `******`。当前后端是整值替换，不支持局部 PATCH；用户保存启用状态或改动 JWT 配置时，前端必须要求重新输入真实 secret，不能提交脱敏占位值。

保存示例：

```http
PUT /api/v1/settings/apiAuth
Content-Type: application/json
...
- `roleMapping.googleHostedDomainToRoles`
- `roleMapping.googleEmailToRoles`
- `security.stateTTLSeconds`

当前 master 临时不暴露 OAuth 登录路由：`/api/v1/auth/oauth2/google/*` 访问会返回 404。`oauthAuth` 只能作为可保存配置，不证明 OAuth 登录能力已对外暴露；前端可以展示配置状态，但不建议提供可用登录能力的暗示。读取响应会对 `providers.google.clientSecret` 脱敏为 `******`，保存时必须提交真实 client secret。

保存示例：

```http
PUT /api/v1/settings/oauthAuth
Content-Type: application/json
...
- 页面加载时 `GET /api/v1/settings` 能按七种已实现 `type` 填充对应模块。
...
- OAuth 配置可保存，但当前登录路由未注册，因此 UI 不暗示 OAuth 登录已可用。
