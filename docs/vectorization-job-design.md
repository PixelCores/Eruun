# Eruun 文档向量化 Job 设计文档

> 状态：Draft / Proposal。本文是向量化 Job 设计稿，当前代码库未暴露 `/api/v1/vectorize/*` 路由。
...
本设计旨在扩展 Eruun 工作流引擎，增加文档向量化 Job 能力，使其能够：
...
import (
    ...
    "github.com/PixelCores/Eruun/pkg/apiserver/infrastructure/storage"
    "github.com/PixelCores/Eruun/pkg/apiserver/infrastructure/vectorize"
)
...
3. **更新 Eruun 配置**
   ```bash
   kubectl edit configmap eruun-config -n eruun-system
   # 添加向量化配置
   ```

4. **重启 API Server**
   ```bash
   kubectl rollout restart deployment eruun -n eruun-system
   ```
