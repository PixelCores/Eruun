...
其中，对外创建应用请求中的 `components[].tmp` 会被解析为 `TemplateRef`，进入 `CreateApplications` 后由 `resolveComponents` 与 `cloneComponentsFromTemplate` 展开为实际组件创建请求。

## 术语边界

- `components[].tmp.id` 是创建应用请求中的模板引用，指向一个已启用模板的应用。
...
- `POST /applications` 支持通过 `components[].tmp.id` 引用已启用模板的应用，并按模板组件克隆生成新应用组件。创建响应的 `resources` 会从本次解析后的第一个主组件 `traits.resources` 推导。
