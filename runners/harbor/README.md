...
失败归档的根 `result.json` 还会保留有界的 `diagnostics`：`failureClass`、稳定的 `failureReason` 和经过脱敏的 `message`。同一诊断摘要会放入 terminal 的 `message`，便于平台在不读取完整 Harbor 日志的情况下区分准备、框架执行和结果采集失败；诊断不携带 Token、凭据或完整原始日志。上传未确认前仍不会发送 terminal。
